...
const Button:React.FC<ButtonProps> = ({size, shape, dis, text, type, color, onClick}) => {
    ...
    return(
        <button type={type} className={`btn ${sizeClass} ${shape} ${colorClass} cursor-pointer px-4 py-4 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed quattrocento-regular`} 
